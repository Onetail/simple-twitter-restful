...
import { TweetColumnInOrderBy } from '../controller/dto/tweet';
...
export function validateTweetPagenationQuery() {
  return async (ctx) => {
    try {
      ctx.query.page = ctx.query.page ? await parseInt(ctx.query.page) : 1;
      ctx.query.count = ctx.query.count ? await parseInt(ctx.query.count) : 10;
      ...
      ctx.validate(
        {
          ...
          order: {
            type: 'enum',
            values: Object.keys(TweetColumnInOrderBy)
              .map((key) => TweetColumnInOrderBy[key])
              .filter(Boolean),
            default: TweetColumnInOrderBy.ID,
            required: false,
            allowEmpty: false,
          },
          sort: {
