...
until MYSQL_PWD=root mysql -u root -h simple-twitter-restful-mysql-master ; do
...
-e "CREATE USER 'slave'@'simple-twitter-restful-mysql-slave' IDENTIFIED BY 'slave'; \
GRANT REPLICATION SLAVE ON *.* TO 'slave'@'simple-twitter-restful-mysql-slave';"

# get master log File & Position

master_status_info=$(MYSQL_PWD=root mysql -u root -h simple-twitter-restful-mysql-master -e "show master status\G")
...
MYSQL_PWD=root mysql -u root \
-e "CHANGE MASTER TO MASTER_HOST='simple-twitter-restful-mysql-master', \
MASTER_USER='root', \
